...
export default function ResetPassword() {
  ...
  function login(eve) {
    ...
    axios
      .post(import.meta.env.VITE_APP_API + "/resetpassword", {password:admin , token:token})
      .then((res) => {
        if (res.status === 200) {
        ...
        <div className="card shadow-sm p-4" style={{ maxWidth: "400px", width: "90%" }}>
          ...
          <form onSubmit={login} className="d-flex flex-column gap-3">
            ...
            <button type="submit" className="btn btn-primary btn-animated py-2 fw-semibold">
             ...
             <Link to="/login" className='text-center text-decoration-none'>If you Changed password Please Login!</Link>
          </form>
        </div>
      </section>
      <ToastContainer />
